...
local savestate = loadSharedAPI("savestate")
...
local playerSongs = {};
playerSongs[savestate.STATE_BLOCK] = "zelda-song2-";
playerSongs[savestate.STATE_PLAYER] = "zelda-song1-";
playerSongs[savestate.STATE_NPC] = "zelda-song3-";
...
local sfx_warp = pm.registerSound(CHARACTER_ZELDA, "zelda-warp.ogg");
...
function zelda.onDraw()
	if(teleportEmitting > 0) then
		...
		--windowDebug("butts")
		if not (mem(0x00B250E2, FIELD_BOOL) or Misc.isPausedByLua()) then
...
function zelda.onInputUpdate()
	if (player.character == CHARACTER_ZELDA and player:mem(0x13E,FIELD_WORD) == 0 and not (mem(0x00B250E2, FIELD_BOOL) or Misc.isPausedByLua())) then
		pm.winStateCheck()
...
function zelda.onKeyUp(keycode)
	if (player.character == CHARACTER_ZELDA) and player:mem(0x13E,FIELD_WORD) == 0 and ((player:mem(0x122, FIELD_WORD) == 0) or (player:mem(0x122, FIELD_WORD) == 500)) then
...
function zelda.onTick()
	if (player.character == CHARACTER_ZELDA) then
		...
		if (player:mem(0x122, FIELD_WORD) == 0) or (player:mem(0x122, FIELD_WORD) == 500) then
