...
local followa = API.load("followa")
...
local punchBox = colliders.Box(0,0,24,24);

--Everything that can be hit by Bowser's punch and minions
local hittables = {};

for _,v in ipairs(defs.NPC_HITTABLE) do
...
function bowser.onInputUpdate()
	if player.character == CHARACTER_BOWSER then
		...
		if(punchTimer > 0 and not (mem(0x00B250E2, FIELD_BOOL) or Misc.isPausedByLua())) then
			player.upKeyPressing = false;
		...
		--Do punch
		if(player.runKeyPressing and not prun and player:mem(0x122, FIELD_WORD) == 0 and player:mem(0x40,FIELD_WORD) == 0) then
			punchTimer = punchDur;
			punchStarted = false;
			player.runKeyPressing = false;
		end
...
function bowser.onTick()
	if player.character == CHARACTER_BOWSER then
		...
		if(hp < 2) then
			...
			if(power > 1) then
				power = 1;
				Misc.loadCharacterHitBoxes(2, player.powerup, pm.getHitboxPath("bowser",1));
			end
		else
			Graphics.sprites.bowser[2].img = nil;
			if(power < 2) then
				power = 2;
				Misc.loadCharacterHitBoxes(2, player.powerup, pm.getHitboxPath("bowser",player.powerup));
			end
		end
		...
		if(not player:mem(0x13E,FIELD_BOOL)) then --We not dead.
			...
			if(punchTimer > 0) then
				...
				if(punchTimer<punchDur*0.8) then
					if(not punchStarted) then --Punch just happened
						...
						if(player.powerup == PLAYER_FIREFLOWER) then		--SPAWN FIREBALL
							...
							n.friendly = true;
							playSFX(42);
							table.insert(fireballs,pnpc.wrap(n));
						elseif(player.powerup == PLAYER_ICE) then			--SPAWN ICEBALL
							...
							n.friendly = true;
							playSFX(25);
							table.insert(iceballs,pnpc.wrap(n));
							...
							eventu.setFrameTimer(8, f, 2); --Spawn 2 more hammers
						end
					end
...
local function drawHUD()
	...
	if(#minionQueue > 0 and minionQueue[1].npc.isValid) then
		local gfxw = npcconfig[minionQueue[1].id].gfxwidth;
		local gfxh = npcconfig[minionQueue[1].id].gfxheight;
		if(gfxw == 0) then
...
function bowser.onNPCKill(eventObj, npc, reason)
	if player.character == CHARACTER_BOWSER then
		if(reason == 9 and (contains(defs.NPC_POWERUP,npc.id) or npc.id == 250) and npc.id ~= 90 and npc.id ~= 186 and npc.id ~= 187 and npc.id ~= 188 and npc.id ~= 273 and colliders.collide(npc,player)) then
		...
		elseif(reason <= 10 and reason ~= 9 and reason ~= 6 and reason ~= 4 and reason ~= 3 and bowser.minionList[npc.id] ~= nil and pnpc.wrap(npc).data.BOWSER_killedByMinion ~= true) then
				--Recruit a minion!
				pushMinion(npc);
				eventObj.cancelled = true;
				playSFX(13);
