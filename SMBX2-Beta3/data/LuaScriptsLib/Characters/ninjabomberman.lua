...
local sfx_death = pm.registerSound(CHARACTER_NINJABOMBERMAN,"death.ogg");
...
function ninjabomberman.onTick()
	if (player.character == CHARACTER_NINJABOMBERMAN) then
		...
		if isInStartMenu then
			gravity(0)
			horikawaTools.freezePlayer(false, false, player.x, player.y)
			Text.printWP("Press run to Exit Level", 200, 200,-5)
		else
			horikawaTools.unFreezePlayer(false, false)
			gravity(oldGravity)
		end
		...
		for _, npcLister in pairs(NPC.get(horikawaTools.hittableNPCs, player.section)) do
			if (npcLister:mem(0x64, FIELD_WORD) == 0) and (npcLister:mem(0x40, FIELD_WORD) == 0) then
				local thePnpc = pnpc.wrap(npcLister)
		...
		for _, hitBox in pairs(hitBoxArray) do			
			...
			if (not hitBox.thePnpc.isValid) or (hitBox.thePnpc:mem(0x122, FIELD_WORD) ~= 0) then
		...
		if player:mem(0x13C, FIELD_DWORD) ~= 0 or player:mem(0x122, FIELD_WORD) == 227 or player:mem(0x122, FIELD_WORD) == 2 then
			player:kill()
			Audio.SfxStop(-1)
		...
		--Quit level
		if inputs2.state[1].run == inputs2.PRESS and isInStartMenu then
			horikawaTools.unFreezePlayer(false, false)
			exitLevel()
		end
	end
