...
local sfx_claw = pm.registerSound(CHARACTER_ULTIMATERINKA, "claw.ogg");
local sfx_fireflower = pm.registerSound(CHARACTER_ULTIMATERINKA, "fireflower.ogg");
local sfx_iceflower = pm.registerSound(CHARACTER_ULTIMATERINKA, "iceflower.ogg");
local sfx_invin = pm.registerSound(CHARACTER_ULTIMATERINKA, "invin.ogg");
local sfx_metalblade = pm.registerSound(CHARACTER_ULTIMATERINKA, "metalblade.ogg");
local sfx_powerstone = pm.registerSound(CHARACTER_ULTIMATERINKA, "powerstone.ogg");
...
function ultimateRinka.onNPCKill(eventObj,killedNPC,killReason)
	if player.character == CHARACTER_ULTIMATERINKA then
		if killReason ~= 9 and horikawaTools.npcList[killedNPC.id] then
