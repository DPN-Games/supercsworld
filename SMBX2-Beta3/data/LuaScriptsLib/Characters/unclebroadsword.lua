...
local sfx = {
	drop = 	pm.registerSound(CHARACTER_UNCLEBROADSWORD, "sfx\\drop.wav"),
	knife = pm.registerSound(CHARACTER_UNCLEBROADSWORD, "sfx\\knife.wav"),
	lunge = pm.registerSound(CHARACTER_UNCLEBROADSWORD, "sfx\\lunge.wav"),
	swipe = pm.registerSound(CHARACTER_UNCLEBROADSWORD, "sfx\\swipe.wav"),
	charging = pm.registerSound(CHARACTER_UNCLEBROADSWORD, "sfx\\charging.wav")
}
...
local rangs = {}					-- Boomerangs thrown by hammer UB
...
local HAMMER_VERTSPEED = 2			-- Speed at which boomerang can be moved vertically
local HAMMER_LIFETIME = 1.8			-- Number of seconds a boomerang will remain before vanishing
...
function unclebroadsword.onInputUpdate()
	if player.character == CHARACTER_UNCLEBROADSWORD then
		pm.winStateCheck()
		-- Alter boomerang path before blocking input
		for _,rang in ipairs(rangs) do
			if rang.isValid then
				if player.upKeyPressing then rang.speedY = -HAMMER_VERTSPEED
				elseif player.downKeyPressing then rang.speedY = HAMMER_VERTSPEED
				else rang.speedY = 0 end
			end
		end
...
local function UpdateAttackState() ---------------------------------------------------------------- Advance attack state timers	
	...
	if submerged() or sliding() or climbing() or mounted() or holding() or pickingup() then
		attack_state = ATKSTATE.NONE
		can_aerial = true
		can_stallnfall = true
		doublejump_ready = true
...
local function AttackPhysics() -------------------------------------------------------------------- Set player speed and sword hitbox
	...
	if 	attack_state >= ATKSTATE.PAUSE2
	and attack_state <= ATKSTATE.LUNGE_CHARGE
	and attack_state ~= ATKSTATE.NONE
	and airborne() and not inforcedanim() then
		player.speedY = 0
	...
	if player.powerup == PLAYER_HAMMER then
		if (attack_state == ATKSTATE.SWIPE2 and attack_timer == DURATION[ATKSTATE.SWIPE2]*2)
		or (attack_state == ATKSTATE.LUNGE_CHARGE and attack_timer == DURATION[ATKSTATE.LUNGE_CHARGE] + LUNGELAG_CHARGE - 1) then
			local rang = pnpc.wrap(NPC.spawn(292, sword_collider.x + sword_collider.width/2, sword_collider.y + sword_collider.height/2, player.section, false, true))
			rang.data.t = 0											-- Timer for parabolic motion
			rang.data.direction = player:mem(0x106, FIELD_WORD)		-- Direction the boomerang is headed in
			rang.data.multiplier = 1								-- Speed multiplier (if charged lunge)
			if attack_state == ATKSTATE.LUNGE_CHARGE then rang.data.multiplier = 2 end
			rang.data.afterimagepos = {}							-- Table of positions for afterimages
			rangs[#rangs + 1] = rang
		end
	end
	...
	-- Set sword hitbox
	if attack_state > ATKSTATE.NONE then
		if sword_collider == nil then sword_collider = colliders.Box(0,0,0,0) end
		if attack_state == ATKSTATE.SWIPE1 or attack_state == ATKSTATE.SWIPE2 then
		...
		if block:mem(0x1c, FIELD_WORD) == 0 and block:mem(0x5a, FIELD_WORD) == 0 then
			...
			elseif not isType(block.id, BLOCK_PLAYERSWITCH) then
				if attack_state >= ATKSTATE.STALLANDFALL then
					...
					if (block.id == 90 or block.contentID ~= 0) and attack_state == ATKSTATE.STALLANDFALL then colliders.bounceResponse(player) end
				else
					block:hit()
				end
			end
	...
	for _,npc in pairs(NPC.getIntersecting(sword_collider.x, sword_collider.y, sword_collider.x + sword_collider.width, sword_collider.y + sword_collider.height)) do
		...
		if not (npc.invincibleToSword or npc.friendly or npc.isHidden) and npc:mem(0x64, FIELD_WORD) == 0 then
			...
			elseif npc.id == 263 then
				...
				if player.powerup == PLAYER_ICE and npcconfig[npc.id].noiceball == 0 then npc:harm(HARM_TYPE_EXT_ICE)
				-- Ignore if a tool, an egg, or Mouser's bomb
				elseif not (npc.id == 96 or npc.id == 134 or isType(npc.id, NPC_CARRYABLETOOL)) then npc:harm(HARM_TYPE_SWORD) end
...
local function BoomerangPhysics() ----------------------------------------------------------------- Perform logic for boomerang projectiles
	-- Cleanup
	local cleanup = true
	for i,rang in ipairs(rangs) do
		if rang.isValid then
			cleanup = false
			break
		end
	...
	for i,rang in ipairs(rangs) do
		if cleanup then
			table.remove(rangs, i)
		elseif rang.isValid then
			-- Parabolic motion
			rang.speedX = HAMMER_MAXSPEED*(1 - HAMMER_MAXSPEED*rang.data.t/2/HAMMER_MAXDIST)*rang.data.direction*rang.data.multiplier
			rang.data.t = rang.data.t + 1
			if math.abs(rang.speedX) > HAMMER_MAXSPEED*rang.data.multiplier then rang.speedX = -HAMMER_MAXSPEED*rang.data.direction end
			
			-- Set frame
			rang:mem(0xe8, FIELD_FLOAT, 0)
			rang.animationFrame = math.floor(rang.data.t%12)/2
			
			-- Remember positions
			if #rang.data.afterimagepos < 4 then
				rang.data.afterimagepos[#rang.data.afterimagepos + 1] = {x = rang.x - 8, y = rang.y - 6}
			else
				for i = 1, #rang.data.afterimagepos-1 do rang.data.afterimagepos[i] = rang.data.afterimagepos[i+1] end
				rang.data.afterimagepos[#rang.data.afterimagepos] = {x = rang.x - 8, y = rang.y - 6}
			end
			
			-- Destroy bricks
			for _,block in pairs(Block.getIntersecting(rang.x, rang.y, rang.x + rang.width, rang.y + rang.height)) do
				-- If block is visible
				if block:mem(0x1c, FIELD_WORD) == 0 and block:mem(0x5a, FIELD_WORD) == 0 then
					if isType(block.id, BLOCK_BRICK) and block.contentID == 0 then block:remove(true) end
				end
			end
			
			-- Expire old boomerangs
			if rang.data.t > HAMMER_LIFETIME*2500/39 then
...
local function ClearBoomerangs() ------------------------------------------------------------------ Erase all boomerangs (NPC-292) from the level
	for _,rang in pairs(NPC.get(292, player.section)) do
		Animation.spawn(63, rang.x + rang.width/2, rang.y + rang.height/2)
		rang:kill(HARM_TYPE_OFFSCREEN)
	end
end
...
function unclebroadsword.onTick()
	if player.character == CHARACTER_UNCLEBROADSWORD and player:mem(0x13E,FIELD_WORD) == 0 then
		...
		-- Physics for Hammer abilty's boomerangs
		if player.powerup == PLAYER_HAMMER then BoomerangPhysics()
		else ClearBoomerangs() end
		-- Draw visual effects
		SpawnAttackGFX()
...
function unclebroadsword.onDraw()
	if player.character == CHARACTER_UNCLEBROADSWORD then
		...
		for _,rang in ipairs(rangs) do
			if rang.isValid then
				for i,pos in ipairs(rang.data.afterimagepos) do
					Graphics.draw {x = pos.x, y = pos.y, type = RTYPE_IMAGE, image = Graphics.sprites.npc[292].img,
									isSceneCoordinates = true, opacity = 0.7*i/(#rang.data.afterimagepos + 1), priority = -45,
...
function unclebroadsword.initCharacter()
	...
	-- Configure boomerangs
	npcconfig[292].width = 32
	npcconfig[292].height = 20
	npcconfig[292].gfxwidth = 48
	npcconfig[292].gfxheight = 32
	npcconfig[292].gfxoffsety = 6
	npcconfig[292].frames = 6
end
function unclebroadsword.cleanupCharacter()
	...
	-- Reconfigure boomerangs
	npcconfig[292].width = 32
	npcconfig[292].height = 32
	npcconfig[292].gfxwidth = 32
	npcconfig[292].gfxheight = 32
	npcconfig[292].gfxoffsety = 0
	npcconfig[292].frames = 4
end
