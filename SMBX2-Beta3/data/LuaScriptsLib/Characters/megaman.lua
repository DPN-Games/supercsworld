...
local chargeType1 = particles.Emitter(-20400,-20000, (Misc.resolveFile("chargeParticles1.ini") or Misc.resolveFile("graphics\\megaman\\chargeParticles1.ini")))
local chargeType2 = particles.Emitter(-20400,-20000, (Misc.resolveFile("chargeParticles2.ini") or Misc.resolveFile("graphics\\megaman\\chargeParticles2.ini")))
...
local runanim = panim.Anim({2,3,2,6});
local rungrabanim = panim.Anim({9,10});
...
local itemSfx = {}
itemSfx[1] = pm.registerSound(CHARACTER_MEGAMAN, "itemgetshort.ogg")
itemSfx[2] = pm.registerSound(CHARACTER_MEGAMAN, "itemgetlong.ogg")
local chargeSfx = pm.registerSound(CHARACTER_MEGAMAN, "charge1.ogg")
local chargeFullSfx = pm.registerSound(CHARACTER_MEGAMAN, "charge2.ogg")
local chargeReleaseSfx = pm.registerSound(CHARACTER_MEGAMAN, "chargerelease.ogg")

local sfx_hurt = pm.registerSound(CHARACTER_MEGAMAN, "hurt.ogg");
local sfx_shoot = pm.registerSound(CHARACTER_MEGAMAN, "shoot.ogg");
local sfx_teleport = pm.registerSound(CHARACTER_MEGAMAN, "teleport.ogg");
local sfx_cursor = pm.registerSound(CHARACTER_MEGAMAN, "cursor.ogg");
local sfx_pause = pm.registerSound(CHARACTER_MEGAMAN, "pause.ogg");
...
local function drawBar(powerup, value)
	local offset = 28
	if powerup > 2 then
		offset = 48
	end
	Graphics.drawImageWP(pm.getGraphic(CHARACTER_MEGAMAN,megamanHud), offset - 8, 20, 5)
	for i=1, value do
...
local function drawHudElements()
	...
	drawBar(1, healthValue)
	if player.powerup > 2 then
		drawBar(player.powerup, megaman.powerUpStuff[player.powerup].left)
	end
end
...
local function checkShots()
	...
	horikawaTools.removeInvalidNPCS(playerFireballs)
	horikawaTools.removeInvalidNPCS(playerHammers)
	for _, j in pairs(playerFireballs) do
		for _, v in pairs(playerHammers) do
			if v.isValid and j.isValid then
				horikawaTools.makeNPCInvisible(v)
				if (j.direction == -1) then
					spawnModifier = 0
				else
					spawnModifier = npcconfig[282].width
				end
				v.x = j.x + spawnModifier
				v.y = j.y + (0.5 * npcconfig[282].height)
			end
		end
	end
	horikawaTools.removeInvalidNPCS(playerJets)
...
local function handleCharge()
	...
	if not isDead and introOver and not isSliding and not inPause and player.powerup <= 2 then
		if player.runKeyPressing then
			...
			if chargeTime > 140 and chargeSfxObject ~= nil then
				if (player:mem(0x106, FIELD_WORD) == -1) then
					assignSpeedX = -1 * npcPowerup.speedX
					if (npcPowerup.id ~= 160) then
						xOffset = -1 * npcPowerup.xOffset
					else
						xOffset = (-1 * npcconfig[160].width)
					end
				else
					assignSpeedX = npcPowerup.speedX
					xOffset = npcPowerup.xOffset
				end
				theFireball = pNPC.wrap(NPC.spawn(108, player.x + xOffset, player.y + 0.5 * npcPowerup.yOffset + (player:mem(0xD0, FIELD_DFLOAT) - 53)*0.5 - 16, player.section))
...
local function handlePowerupLogic()
	...
	if health <= 4 and player.powerup == 2 then
		player.powerup = 1
	elseif health > 4 and (player.powerup == 1 or player:mem(0x122, FIELD_WORD) == 1) then
		player.powerup = 2
	end
	if health > 28 then
		health = 28
	...
	if disableTable[player:mem(0x122, FIELD_WORD)] then
...
function megaman.onInitAPI()
	...
	registerEvent(megaman, "onDraw", "onDraw", false)
...
function megaman.onDraw()
	if player.character == CHARACTER_MEGAMAN then
		...
		elseif(vanillaFrame == 8) then
			player:mem(0x114,FIELD_WORD, 11);
		elseif(vanillaFrame == 9) then
			player:mem(0x114,FIELD_WORD, 12);
		elseif(vanillaFrame == 10) then
	...
	if player.character == CHARACTER_MEGAMAN then
		if isSliding then
			for _, v in pairs(NPC.getIntersecting(player.x - 8, player.y - 8, player.x + player.width + 8, player.y + player.height + 8)) do
				if v.id == 91 then
					theGrassContents = NPC.spawn(v:mem(0xF0, FIELD_DFLOAT), v.x, v.y-32, player.section)
					theGrassContents.speedX = rng.randomInt(-2,2)
					v:kill()
					playSFX(88)
				end
			end
...
function megaman.onKeyDown(keycode)
	if player.character == CHARACTER_MEGAMAN then
		if(player:mem(0x13E, FIELD_WORD) > 0) or (player:mem(0x122, FIELD_WORD) ~= 0) then
			return;
		end
		...
		if (keycode == KEY_X) and not isDead and introOver and not introDelay and not isSliding and not inPause then
			if megaman.powerUpStuff[player.powerup].left > 0 or player.powerup <= 2 then
				...
				theFireball = pNPC.wrap(NPC.spawn(npcPowerup.id, player.x + xOffset, player.y - 8 + npcPowerup.yOffset + (player:mem(0xD0, FIELD_DFLOAT) - 53)*0.5, player.section))
				...
				if (player.powerup == 3) then
					...
					theHammer = pNPC.wrap(NPC.spawn(171, theFireball.x + spawnModifier, theFireball.y + (0.5 * npcconfig[282].height), player.section))
...
function megaman.initCharacter()
	...
	npcconfig[108].gfxwidth = 64;
	npcconfig[108].gfxheight = 60;
	npcconfig[108].height = 64;
	npcconfig[108].width = 60;
	npcconfig[108].frames = 3;
	npcconfig[108].framestyle = 1;
end
...
function megaman.cleanupCharacter()
	...
	npcconfig[108].gfxwidth = 32;
	npcconfig[108].gfxheight = 32;
	npcconfig[108].height = 32;
	npcconfig[108].width = 32;
	npcconfig[108].frames = 2;
	npcconfig[108].framestyle = 1;
