...
local function findMePeterPan(rDirection)
	...
	for i, j in pairs(localNpcList) do
		if (j.isValid and defs.NPC_HITTABLE_MAP[j.id] and j.id ~= 17 and j.id ~= 18 and not j.friendly and j:mem(0x124, FIELD_BOOL) and not j:mem(0x64, FIELD_BOOL) and not j.isHidden) then
	...
	if #realNpcList ~= 0 then
		...
		if(targettedNPC ~= nil and targettedNPC.isValid) then
			for i=1,#realNpcList do
				if(realNpcList[i] == targettedNPC.__ref) then
					targetIndex = i;
					break;
				end
			end
		...
		targettedNPC = pNPC.wrap(realNpcList[targetIndex])
	...
	if (player.character == CHARACTER_ROSALINA) then
		...
		if (keycode == KEY_SEL) then
			...
			if (player.powerup == 6) then
				...
				if #gotBombs >= 1 then
					local bomb = pNPC.wrap(gotBombs[1])
					bomb.data.drop = true
				end
				--for _, fireball in pairs(NPC.get(projectileType[player.powerup], player.section)) do
					--butts = pNPC.wrap(fireball)
		...
		if hasFoundPeterPan then
			if inputs2.state[1].dropitem == inputs2.HOLD then
...
local function handleJumps()
	if (player:mem(0x146, FIELD_WORD) == 0) and (player:mem(0x48, FIELD_WORD) == 0) then
		...
		if  inputs2.state[1].jump == inputs2.HOLD or inputs2.state[1].altjump == inputs2.HOLD then
...
function rosalina.onDraw()
	if(player.character == CHARACTER_ROSALINA) then
		Graphics.sprites.hardcoded["33-2"].img = pm.getGraphic(CHARACTER_ROSALINA,starbitcounter);
		if (player.powerup == 3 or player.powerup == 7) then
...
function rosalina.onInputUpdate()
	if player.character == CHARACTER_ROSALINA then
		pm.winStateCheck()
		if(not mem(0x00B250E2, FIELD_BOOL) and not Misc.isPausedByLua()) then
			if(player.runKeyPressing or player.altRunKeyPressing) then
				if(not pressedRun) then
					if(player.powerup == PLAYER_HAMMER and player.holdingNPC == nil) then
						local n = pNPC.wrap(NPC.spawn(projectileType[PLAYER_HAMMER], player.x+player.width*0.5, player.y, player.section));
						n.x = n.x-n.width*0.5;
