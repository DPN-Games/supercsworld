...
local sfx_missileHit = pm.registerSound(CHARACTER_SAMUS, "missileHit.ogg");
local sfx_shoot = pm.registerSound(CHARACTER_SAMUS, "samus_shoot_sound.ogg");
local sfx_shootice = pm.registerSound(CHARACTER_SAMUS, "samus_shoot_sound_ice.ogg");
local sfx_shootmissile = pm.registerSound(CHARACTER_SAMUS, "samus_shoot_sound_missile.ogg");
		...
		for _, v in ipairs(shotNPCs) do
			if(v.isValid) then
				...
				elseif v.id == 171 then
					...
					if colliders.collideBlock(v, defs.BLOCK_SOLID..defs.BLOCK_NONSOLID..defs.BLOCK_LAVA..defs.BLOCK_PLAYER, blockFilter) then
						v:kill(9)
						Animation.spawn(108, v.x, v.y)
	...
	if (player.character == CHARACTER_SAMUS) then
		...
		if messaged == false then
			if (state.isMorphBall) then
				...
				if (player.runKeyPressing and not wasRunKeyPressing and player:mem(0x122, FIELD_WORD) == 0) then
					samus.fireGun()
				end
			...
			if (player.downKeyPressing and not wasDownKeyPressing and downwarp ~= true and player:mem(0x122, FIELD_WORD) == 0 and ceiling == false) then
...
function samus.fireGun()
	if messaged == false then
		...
		table.insert(shotNPCs, pnpc.wrap(shot));
		if player.powerup == 5 then
			if (dir >= 0) then
				shot.speedX = 9
			else
...
local function onBombTick(bomb)
	...
	if (bomb.tick > 60) then
		...
		for _, hurtableNPC in pairs (NPC.getIntersecting(bomb.x-32, bomb.y-32, bomb.x+32, bomb.y+32)) do
			if horikawaTools.npcList[hurtableNPC.id] and not hurtableNPC.friendly then
				hurtableNPC:kill(3)
			end
...
function samus.onDraw()
	...
	while i <= #shotNPCs do
		if(shotNPCs[i].isValid) then
			...
			local frame = v.animationFrame + npcconfig[v.id].frames*(math.max(v.direction,0));
...
function samus.onNPCKill(eventObj, killingNPC, killReason)
	if (killReason == 10 or killReason == 1 or killReason == 2 or killReason == 3) and player.character == CHARACTER_SAMUS and defs.NPC_HITTABLE_MAP[killingNPC.id] then
