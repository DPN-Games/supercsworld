...
-- API init --
local configFileReader = API.load("configFileReader",true)
local timerApi = API.load("leveltimer",true)
local LsBreathMeter = API.load("LsBreathMeter",true)
local autoscroll = API.load("autoscroll",true)
...
function tweaks.onStart()
	if Misc.resolveFile("tweaks.ini") ~= nil then
		...
		if tweakFile.timer ~= nil then
			timerApi.setSecondsLeft(tweakFile.timer);
			timerApi.setTimerState(true);
			timerApi.willKillPlayer = false;
		end
		...
		if tweakFile.smwCamera == 1 then
			local SMWcamera = API.load("Tweaks//SMWcamera")
		end
...
function tweaks.onTick()
	if Misc.resolveFile("tweaks.ini") ~= nil then
		if tweakFile.timer ~= nil and timerApi.outtatime == true and triggeredTweak == false then
			triggeredTweak = true;
			if tweakFile.timerResult == 2 then
				triggerEvent(tweakFile.timerEvent)
			elseif tweakFile.timerResult == 3 then
				Level.exit()
			else
				timerApi.willKillPlayer = true;
			end
		end
