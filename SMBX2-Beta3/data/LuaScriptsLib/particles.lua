...
function particles.Grad(points, vals, res)
	...
	for k,v in ipairs(points) do
		g[math.floor(v*res)] = vals[k];
		table.insert(g.keys,math.floor(v*res))
	end
...
function gradient:get(x)
	x = clamp(math.floor(x*self.resolution),0,self.resolution);
	if(self[x] == nil) then
...
function parseValue(l,stack)
	...
	elseif(string.match(l,"^%s*{.+}%s*,%s*{.+}%s*,%s*%d+%s*$")) then --Gradient with Resolution
		local pts,vals,res = string.match(l,"^%s*{(.+)}%s*,%s*{(.+)}%s*,%s*(%d+)%s*$");
		pts = parseList(pts,stack);
		for k,v in ipairs(pts) do
			pts[k] = tonumber(v);
			if(pts[k] == nil) then
...
function emitter:Draw(priority)
	...
	Graphics.glDraw{texture=self.texture, vertexCoords=tris, textureCoords=txs, vertexColors=cols, priority=priority, sceneCoords=true}
end
