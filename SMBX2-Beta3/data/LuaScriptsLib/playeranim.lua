...
function playerAnim.getFrameRaw(p, absolute)
	...
	if(absolute) then
		...
		if(p:mem(0x106,FIELD_WORD) > 0) then
			...
			return 4-math.floor((base-1)/10), 9-(base-1)%10;
		end
	else
...
function playerAnim.onDraw()
	for k,v in pairs(animSet) do
		local dirmod = 1;
		if(v.anim.start ~= nil) then
			if(v.anim.stop < v.anim.start) then
				dirmod = -1;
			end
		end
		if(v.anim.speed > 0) then
			...
			if(v.counter >= v.anim.speed) then
				if(v.anim.start ~= nil) then
					v.frame = (v.frame+1)%(math.abs(v.anim.stop-v.anim.start)+1);
				else
		...
		if(queue[k] == nil) then
			if(v.anim.start ~= nil) then
				queue[k] = {v.anim.start + v.frame*dirmod, v.anim.absolute};
			else
