...
--***************************************************************************************************
	...
	local function HSLToRGBAHex (hue, saturation, lightness, alpha)
		...
		return 0xFF000000*(r+m) + 0x00FF0000*(g+m) + 0x0000FF00*(b+m) + 0x000000FF*alpha
	end
	...
	function textblox.formatTextForWrapping (text, font, wrapWidth, addSlashN)
		...
		for textChunk in string.gmatch(text, "<*[^<>]+>*")	do
			...
			if  string.find(textChunk, "<.*>") ~= nil  then
				...
				if  checkForWrap  then
					if  replaceString ~= nil  then  addWidth = font:getStringWidth (replaceString);  end;
			...
			-- Is plaintext
				...
				while (i <= #words) do
					...
					local word = words[i]
					local overflowPos = font:getStringWrapPoint (word, math.max(0, wrapWidth-currentLineWidth))
					
					-- If not overflowing, just append and move on
					if overflowPos == nil  then
						newTextChunk = newTextChunk..word
						currentLineWidth = currentLineWidth + font:getStringWidth (word)
					...
					-- Otherwise...
						...
						local overflowPos = font:getStringWrapPoint (word, math.max(0, wrapWidth-currentLineWidth))
						local timesSplit = 0
						local wordWidth = font:getStringWidth (word)
						...
						elseif  isWhitespace  then
							...
							if  word == " "  then  
								...
								wordWidth = font:getStringWidth (secondStr)
							else
						...
						-- Otherwise, begin the hypenating loop
							...
							while (overflowPos ~= nil)  do
								...
								overflowPos = font:getStringWrapPoint (word, wrapWidth)
							...
							newTextChunk = newTextChunk..word
							wordWidth = font:getStringWidth (word)
						end
...
do
	...
	------ Methods ---------------------------------------------------------------------------------------------
	function Font:getStringWidth (text, isMonospace)
		local totalWidth = 0		
		if  text == nil  or  text == ""  then  return totalWidth;  end
		
		-- Loop through each character
		for character  in  string.gmatch(text, ".")	do
			totalWidth = totalWidth + self:getCharWidth (character, isMonospace) + self.kerning
		end
	...
	function Font:getCharWidth (character, isMonospace)
	...
	function Font:getHeight ()
		return self.charHeight*self.scaleY
	end
	
	
	function Font:getStringWrapPoint (text, width, isMonospace)
		local totalWidth = 0		
		local currentPos = 0
		local currentChar = ""
		local prevChar = ""
		...
		if  self:getStringWidth (text) < width  then  return nil, nil, nil;  end
...
do
	...
	function textblox.printExt (text, properties)
		...
		if  (width ~= math.huge  and  prewrapped ~= true)  then
			text = textblox.formatTextForWrapping (text, font, width, false)
		end
				...
				string.gsub (textChunk, ".", function(c)
					...
					if  c ~= " "  or  iconMode  then
						...
						if	t_valign == textblox.VALIGN_BOTTOM  then
							yPos = y + (lineBreaks - totalLineBreaks - 1)*font:getHeight ()

						elseif t_valign == textblox.VALIGN_MID  then
							yPos = y + (lineBreaks*font:getHeight()) - ((totalLineBreaks+1)*font:getHeight()*0.5)
						end
						...
						if  rainbowMode == true  then
							currentColor = HSLToRGBAHex (textblox.waveModeCycle+totalShownChars, 0.5, 0.5, 1)
						end
...
do		
	function TextBlock.create(x,y, textStr, properties, template)		
		...
		if  (thisTextBlock.pauseGame == true)  then  Misc.pause ();  end;
	...
	function TextBlock:updateIdealWidth (ratio)
		if  self.autosizeDirty  then
			...
			-- Setup
			local fullTextWidth = self.font:getStringWidth (self.text)
			local minWidth = self.font:getStringWidth ("WWWWWWWWWWWWWWWWWW")
	...
	function TextBlock:updateFormattedText ()
		if  self.textDirty  then
			...
			self.wrappedText = textblox.formatTextForWrapping (self.text, self.font, self.idealWidth, false)
	...
	function TextBlock:draw ()
		...
		local shouldShowIcon = (self:getFinished()  and  self.showNextIcon)
